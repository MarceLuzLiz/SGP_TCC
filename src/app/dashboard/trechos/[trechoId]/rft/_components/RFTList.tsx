...
import { FilePenLine, Trash2, FileText, Loader2 } from 'lucide-react';
...
export function RFTList({ relatorios, trechoId }: RFTListProps) {
  ...
  const handleDelete = (e: React.MouseEvent, relatorioId: string) => {
    e.stopPropagation();
    if (confirm('Tem certeza que deseja excluir este RFT?')) {
      startTransition(async () => {
        const result = await deleteRelatorio(relatorioId, trechoId, 'RFT');
        if (result.error) {
          toast.error(`Erro: ${result.error}`);
        } else {
          toast.success(result.success || 'RFT excluído com sucesso!');
        }
      });
    }
  };
    ...
    <div className="rounded-2xl border border-slate-200/90 bg-white dark:bg-slate-900 shadow-xs overflow-hidden">
      <div className="p-4 md:px-6 border-b border-slate-100 dark:border-slate-800">
        <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200">
          {relatorios.length} {relatorios.length === 1 ? 'RFT registrado' : 'RFTs registrados'}
        </h2>
      </div>
      ...
      {relatorios.length > 0 ? (
              ...
              <div className="grow flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-sm text-slate-900 dark:text-slate-100 group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors">
                    RFT de {new Date(relatorio.createdAt).toLocaleDateString('pt-BR')}
                  </p>
        ...
        <p className="p-8 text-center text-sm text-muted-foreground">Nenhum RFT registrado.</p>
      )}
    </div>
  );
}