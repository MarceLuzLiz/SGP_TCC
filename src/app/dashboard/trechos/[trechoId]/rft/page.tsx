...
import { RFTList } from './_components/RFTList';
...
async function getRFTData(trechoId: string, from?: string, to?: string, statuses?: string[]) {
  const whereClause: Prisma.RelatorioWhereInput = { tipo: 'RFT', trechoId: trechoId };
...
export default async function RFTListPage({ 
  params, 
  searchParams 
}: { 
  ...
  return (
    <div>
      <nav className="mb-6 flex items-center text-sm text-gray-500">
        ...
        <span className="font-semibold text-gray-700">Relatórios Fotográficos (RFT)</span>
      </nav>
      
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-3xl font-bold text-gray-800">Relatórios Fotográficos (RFT)</h1>
        <Link href={`/dashboard/trechos/${resolvedParams.trechoId}/rft/new`} className="flex items-center gap-2 rounded-lg bg-teal-600 px-4 py-2 text-white transition hover:bg-teal-700">
          <FilePlus size={18} />
          Criar Novo RFT
        </Link>
      </div>
      ...
      <RFTList relatorios={trecho.relatorios} trechoId={trecho.id} />
    </div>
  );
}