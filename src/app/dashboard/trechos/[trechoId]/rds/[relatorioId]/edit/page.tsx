...
import { RDSEditor } from '../../_components/RDSEditor';
...
export default async function RDSEditPage({ params }: { params: Promise<{ trechoId: string, relatorioId: string }> }) {
  const resolvedParams = await params;
  const data = await getEditData(resolvedParams.relatorioId);

  if (!data || data.relatorio.tipo !== 'RDS') notFound();

  if (data.relatorio.statusAprovacao !== StatusAprovacao.PENDENTE && data.relatorio.statusAprovacao !== StatusAprovacao.REPROVADO) {
    redirect(`/dashboard/trechos/${resolvedParams.trechoId}/rds/${resolvedParams.relatorioId}`);
  }
  ...
  return (
    <div>
      <nav className="mb-6 flex items-center text-sm text-gray-500">
        ...
        <span className="font-semibold text-gray-700">Editar RDS</span>
      </nav>
      
      <div className="p-6 bg-white rounded-lg shadow-md">
        <h1 className="text-2xl font-bold text-gray-800 mb-4">Editar Relatório Diário de Serviço</h1>
        <RDSEditor
