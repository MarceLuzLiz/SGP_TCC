...
export function RDSList({ relatorios, trechoId }: RDSListProps) {
  ...
  const handleDelete = (e: React.MouseEvent, relatorioId: string) => {
    ...
    if (window.confirm('Tem certeza que deseja excluir este RDS?')) {
  ...
  return (
    <div className="rounded-lg bg-white shadow-md">
      <div className="p-4 border-b"><h2 className="text-lg font-semibold">{relatorios.length} RDSs registrados</h2></div>
      {relatorios.length > 0 ? (
          ...
          {relatorios.map((relatorio) => (
            ...
            >
              <div className="flex-grow">
                <p className="font-semibold text-gray-800">RDS de {new Date(relatorio.createdAt).toLocaleDateString('pt-BR')}</p>
        ...
        <p className="p-4 text-center text-gray-500">Nenhum RDS registrado.</p>
      )}
    </div>
  );
}