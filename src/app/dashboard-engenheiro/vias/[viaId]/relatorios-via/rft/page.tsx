...
export default async function RftViaListPage({
  params,
  ...
  const { from, to } = await searchParams; // 4. Ler filtros
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">RFTs Consolidados da Via</h1>
          <p className="text-muted-foreground">Via: {via.name}</p>
        </div>
        <Button asChild>
          <Link href={`/dashboard-engenheiro/vias/${viaId}/relatorios-via/rft/novo`}>
            <PlusCircle className="mr-2 h-4 w-4" />
            Criar Novo RFT da Via
          </Link>
        </Button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {relatorios.length === 0 && (
          <p className="text-muted-foreground col-span-full">Nenhum RFT consolidado criado.</p>
        )}
        {relatorios.map(rel => (
          <Card key={rel.id} className="hover:border-primary transition-colors relative">
            ...
            <div className="absolute top-2 right-10 z-10">
              <DownloadConsolidadoViaButton id={rel.id} type="RFT_VIA" />
            </div>
            
            <Link href={`/dashboard-engenheiro/vias/${viaId}/relatorios-via/rft/${rel.id}`}>
              <CardHeader>
